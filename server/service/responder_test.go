...
func TestResponse_ToClientString(t *testing.T) {
	response := NewResponse("val", Key("testKey"), Value("veryLongValue"), "connection1")
...
func TestResponse_ToClientStringAcknowledged(t *testing.T) {
	response := NewResponse("ack", Key("testKey"), Value("veryLongValue"), "connection1")
	message := response.ClientString()
	if message != "ack" {
...
func TestResponse_ToClientStringFailed(t *testing.T) {
	response := NewResponse("nil", Key("testKey"), Value("veryLongValue"), "connection1")
	message := response.ClientString()
	if message != "nil" {
